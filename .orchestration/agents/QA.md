...
1. **Test Scenario Matrix (Excel-compatible)**
   - Format: CSV
   - File: `test/qa/TEST_SCENARIOS_<YYYY-MM-DD>.csv`
2. **Execution Report (Documentation)**
   - Format: Markdown
   - File: `test/qa/QA_REPORT_<YYYY-MM-DD>.md`
3. **Defect Log (Excel-compatible)**
   - Format: CSV
   - File: `test/qa/DEFECT_LOG_<YYYY-MM-DD>.csv`
...
- Save all evidence and command outcomes in `test/qa/QA_REPORT_<YYYY-MM-DD>.md`
